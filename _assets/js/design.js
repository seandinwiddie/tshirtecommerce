...
var design = {
    ...
    ini: function () {
        ...
        $jd('.ui-lock').click(function () {
            ...
            if ($jd(this).is(':checked') == true)
                self.item.resize(e, 'n, e, s, w, se');
            else
                self.item.resize(e, 'se');
        });
        ...
        $jd('.menu-left a').click(function () {
            ...
            if ($jd(this).hasClass('add_item_text'))
                self.text.create();
            if ($jd(this).hasClass('add_item_team'))
                self.team.create();
            if ($jd(this).hasClass('add_item_qrcode'))
                self.qrcode.open();
        ...
        $jd('.add_item_productDesing').click(function () {
            self.designer.art.categories(true, 0);
            if (jQuery('#dag-list-arts').html() == '')
                self.designer.art.arts('');
        });   
        ...
        $jd('.add_item_productShapes').click(function () {
            self.designer.art.categories(true, 0);
            if (jQuery('#dag-list-shapes').html() == '')
                self.designer.art.shapes();
        });           
        ...
        $jd('#dag-shape-detail button').click(function () {
            jQuery('#dag-shape-detail').hide('slow');
            jQuery('#dag-list-shape').show('slow');
            jQuery('#shape-add').hide();
            jQuery('#shape-pagination').css('display', 'block');
        });
    ...
    storeState: function (e) {
        if(design.importingUndo) return;
        var current = design.exports.vector();
        var strCurrent = JSON.stringify(current);
        if (strCurrent !== design.steps[design.step - 1] && strCurrent !== design.steps[design.step]  && typeof current.front !== 'undefined' && typeof current.front[0] !== 'undefined') {
            //console.log('changes saved');
            design.step = design.step + 1;
            design.steps[design.step] = strCurrent;
            
            design.firstUndo = true;
        }
    },
    ajax: {
        form: function () {
        ...
        addJs: function (e) {
            if(jQuery(e).attr('data-type') !== 'undefined' && jQuery(e).attr('data-type') == "verifyClick"){
                var totalQuantity = 0;
                jQuery('.p-color-sizes input').each(function(){
                    if(parseInt(jQuery(this).val())){
                        totalQuantity= parseInt(jQuery(this).val()) + totalQuantity;
    ...
    tools: {
        ...
        preview: function (e)
        {
            ...
            setTimeout(function () {
                ...
                if (jQuery('#view-left .product-design').html() != '') {
                    ...
                    {
                        jQuery('#slide-left').append(design.output.back);
                    }
                }
                ...
                if (jQuery('#view-right .product-design').html() != '') {
                    ...
                    {
                        jQuery('#slide-right').append(design.output.back);
                    }
                }
        ...
        undo: function (e)
        {
            ...
            if (design.step >= 1) {
                ...
                if (design.firstUndo && typeof design.steps[design.step - 2] !== 'undefined') {
                    minus = 2;
                    design.firstUndo = false;
                }
                */
                if (typeof design.steps[design.step - minus] !== 'undefined') {
                   
                    delete design.steps[design.step];
                    /*
                    if(minus == 2){
                        delete design.steps[design.step - 1];
                    }
                    */
                   
                    console.log( design.steps);
                    design.step = design.step - minus;
                    console.log(design.step);
                    var str = design.steps[design.step];
                    design.importingUndo = true;
                    design.tools.hardReset();                 
                    design.imports.vector(str);
                    design.importingUndo = false;
                }else
                {
                     console.log('Invalid back step' + minus);
                }
            }else
            {
                console.log('No more steps to return');
            }
        },
        redo: function (e)
        {
        ...
        size: function () {
            ...
            jQuery.each(postions, function (i, postion) {
                if (jQuery('#view-' + postion + ' .content-inner').html() != '' && jQuery('#view-' + postion + ' .product-design').html() != '')
                {
                    ...
                    {
                        if (print.width >= area.width)
                            print.width = area.width;
                        if (print.height >= area.height)
    ...
    designer: {
        art: {
            ...
            categories: function (load, index) {
                if (jQuery('#dag-art-categories').length == 0)
                    return false;
            ...
            loadarts: function (cate_id, start, keyword) {
                ...
                if (arts.length > 0)
                {
                    ...
                    while (start - i < limit)
                    {
                        ...
                        if (cate_id > 0)
                        {
                            if (typeof art.cate_id == 'undefined')
                                art.cate_id = 0;
                        ...
                        a.item = art;
                        var img = '<img width="150px" alt="" src="' + url + art.thumb + '">';
                        a.innerHTML = img;
                        div.appendChild(a);
            ...
            {
                if (jQuery('#dag-list-arts').length == 0)
                    return false;
            ...
            shapes: function(){
                ...
                jQuery('#dag-list-shapes').show('slow');
                
                jQuery.ajax({
                    dataType: "json",
                    url: siteURL + "ajax.php?type=shapes"
                }).done(function (data) {
                    if (data == null || data.count == 0)
                    {
                        jQuery('#dag-list-shapes').removeClass('loading');
                        parent.innerHTML = lang.designer.datafound;
                        var ul = jQuery('#shapes-pagination .pagination').html('');
                        return false;
                    }
                   // var keyword = jQuery('#art-keyword').val();
                    var keyword = '';
                    ...
                    self.loadShapes(0,keyword);
                    jQuery('#dag-list-shapes').removeClass('loading');
                });                
            },
            loadShapes: function(start,keyword){
                var arts = this.shapes;
                
                var parent = document.getElementById('dag-list-shapes');
                var count = arts.length;
                ...
                if (count > 0)
                {
                    jQuery('#dg-shapes .modal-body').css('opacity', '0.5');
                    var limit = start + 1800;
                    var i = 0;
                    while (start - i < limit)
                    {
                        ...
                        if (typeof arts[start] == 'undefined')
                        {
                            jQuery('#shapes-pagination').css('display', 'none');
                            count = 0;
                            break;
                        }
                        var art = arts[start];
                    ...
                    if (count > 1)
                    {
                        jQuery('#shapes-pagination').css('display', 'block');
                        var ul = jQuery('#shapes-pagination .pagination');
                        var button = document.createElement('button');
                        button.className = 'btn btn-primary btn-sm';
                        button.setAttribute('type', 'button');
                        button.innerHTML = lang.text.show_design;
                        ...
                        jQuery(button).click(function () {
                            ...
                            // fix layout clipart.
                            setTimeout(function () {
                                var wboxart = jQuery('#dag-list-shapes').width();
                                if (wboxart > 415)
                                    var countart = parseInt(wboxart / 162);
                                else
                                    var countart = parseInt(wboxart / 152);
                                jQuery('#dag-list-shapes').find('.box-art').css('width', parseInt(wboxart / countart - 13) + 'px');
                                jQuery('#dg-cliparts .modal-body').css('opacity', '1');
                            }, 100);
                        });
                    }
            ...
            shapeDetail: function (e)
            {
                ...
                jQuery('#shapes-pagination').css('display', 'none');
                if (document.getElementById('shape-detail-' + id) == null)
                {
                    var div = document.createElement('div');
                    div.className = 'box-art-detail';
                    div.setAttribute('id', 'shape-detail-' + id);
                    var html = '<div class="col-xs-5 col-md-5 shape-detail-left">'
                            + '<img class="thumbnail img-responsive" src="' + jQuery(e).data('medium') + '" alt="">'
                            + '</div>'
                            + '<div class="col-xs-7 col-md-7 shape-detail-right">'
                            + '</div>';
                    div.innerHTML = html;
                    jQuery('#dag-shape-detail').append(div);
                    jQuery('.shape-detail-price').html('');

                    var info = jQuery('#shape-detail-' + id + ' .shape-detail-right');
                    info.html('');
                ...
                {
                    jQuery('#shape-detail-' + id).css('display', 'block');
                }
                jQuery('.shape-detail-price').html(lang.text.fromt + ' ' + currency_symbol + jQuery(e).data('price'));
                ...
                jQuery('#dag-shape-detail').show('slow');
                jQuery('#shape-add').show();
                jQuery('#shape-add button').unbind('click');
                jQuery('#shape-add button').bind('click', function (event) {
                    design.art.create(e);
                });
                jQuery('#shape-add button').button('reset');
                */
            },
            ...
            artDetail: function (e)
            {
                ...
                if (document.getElementById('art-detail-' + id) == null)
                {
                    ...
                    var html = '<div class="col-xs-5 col-md-5 art-detail-left">'
                            + '<img width="250px" class="thumbnail img-responsive svgResponsive" style="" src="' + jQuery(e).data('medium') + '" alt="">'
                            + '</div>'
    ...
    products: {
        ...
        sizes: function () {
            ...
            if (sizes < 0)
                sizes = 0;
            if (typeof min_order !== 'undefined' && min_order != '')
            {
                if (sizes == 0)
                    sizes = parseInt(min_order);
            }
            if (check)
            {
        ...
        changeDesign: function (e) {
            ...
            jQuery.each(product.design.color_hex, function (i, color) {
                ...
                var span = document.createElement('span');
                if (i == 0)
                    span.className = 'bg-colors dg-tooltip active';
                else
    ...
    text: {
        getValue: function () {
            ...
            if ($jd('#font-style-bold').hasClass('active'))
                o.fontWeight = 'bold';
    ...
    art: {
        create: function (e) {
            ...
            if(typeof jQuery(e).data('isShape') !== 'undefined' && jQuery(e).data('isShape')){
                if(typeof jQuery('.isShape')[0] !== 'undefined'){
                    design.layers.remove(jQuery('.isShape').attr('id').replace('item-',''));
                }
                o.isShape = true;
                var setWidth =jQuery('.design-area').width();
                var setHeight = jQuery('.design-area').height();
                if(setHeight > 0){
                ...
                if(setWidth > 500){
                    var diff = setWidth - 500 + 1;
                    setHeight = setHeight - diff;
                    setWidth = setWidth-diff;                    
                }
                
                isShape = '&width=' + setWidth + '&height=' + setHeight;
            ...
            } else
            {
                $jd.ajax({
                    type: "POST",
                    data: item,
                    url: siteURL + "ajax.php?type=svg" + isShape,
                    dataType: "json",
                    success: function (data) {
                        ...
                        if(isShape.length){
                            ...
                            jQuery(e).addClass('isShape');
                            jQuery(e).css('left','0px','important');
                            jQuery(e).css('top','0px','important');
                        }
                        design.item.setup(e[0].item);
                        if(isShape.length){
                            design.layers.sort();
    ...
    item: {
        ...
        create: function (item) {
            ...
            if(typeof item.isShape === 'undefined'){
                item.edit = false;
            }
            this.unselect();
        ...
        select: function (e) {
            if (jQuery(e).hasClass('drag-item-selected') == true)
                return false;
            this.unselect();
            jQuery('.labView.active .design-area').css('overflow', 'visible');
            ...
            setTimeout(function(){
                //console.log(jQuery(e).attr('id').replace('item-', ''));
             if(jQuery(e).hasClass('isShape') || jQuery(e).attr('id').replace('item-', '') == 0){
                //console.log('hiding');
                jQuery('.mask-items-view').hide();
            }else
            {
                jQuery('.mask-items-view').show();
            }
    ...
    layers: {
        ...
        add: function (item) {
            ...
            jQuery('#dg-popover #clipart-lock').click();
        },
        remove: function (id) {
    ...
    popover: function (e) {
        ...
        var position = jQuery('.menu-left .' + e).position();
        ...
        items: function (postion, callback)
        {
            ...
            jQuery('#view-' + postion + ' .design-area .drag-item').each(function () {
                ...
                if(jQuery(this).hasClass('isShape')){
                    obj[i].left = jQuery('.isShape').position().left;
                    obj[i].top = jQuery('.isShape').position().top;
            ...
            jQuery.each(postions, function (i, postion) {
                if (jQuery('#view-' + postion + ' .product-design').html().length > 10)
                {
                    ...
                    jQuery('#view-' + postion).find('.drag-item').each(function () {
                        ...
                        if(jQuery(this).hasClass('isShape')){
                            item.left = jQuery('.isShape').position().left;
                            item.top  = jQuery('.isShape').position().top;
        ...
        loadDesign: function (key, user_id) {
            ...
            jQuery.ajax({
                ...
                } else
                {
                    ...
                    jQuery('#item-0').addClass('isShape');
...
(function ($) {
    ...
    setTimeout(function(){
        ...
        if(!$.urlParam('id') && !$.urlParam('cart_id')){
            createDefault();
        }
    },1500);
...
function createDefault(){
    var el = JSON.parse('{"item":{"fle_url":"","file_name":"dg-designer-3aaa3db614796958424435136821047623.svg","file_type":"svg","colors":"","path":"http://www.uscustomstickers.com/tshirtecommerce/uploaded/cliparts/","url":"http://www.uscustomstickers.com/tshirtecommerce/uploaded/cliparts/517/","change_color":1,"thumb":"print/dg-designer-3aaa3db614796958424435136821047623.svg","medium":"print/dg-designer-3aaa3db614796958424435136821047623.svg","description":"Use this Perfect Circle With Cut Line if you want to make a circle decal. Fill color and border color can both be changed. The outer magenta line is the cut line and will not be printed. Keep all art inside the cut line. Any art outside the cut line will not be printed.","title":"Perfect Circle With Cut Line","price":0,"cate_id":"517","clipart_id":857,"imgThumb":"http://www.uscustomstickers.com/tshirtecommerce/uploaded/cliparts/517/print/dg-designer-3aaa3db614796958424435136821047623.svg","imgMedium":"http://www.uscustomstickers.com/tshirtecommerce/uploaded/cliparts/517/print/dg-designer-3aaa3db614796958424435136821047623.svg"}}');
    jQuery(el).data('isShape',true);
    design.art.create(el);
